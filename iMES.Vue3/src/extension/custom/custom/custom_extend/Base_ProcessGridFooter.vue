...
export default {
  ...
  data() {
    return {
      columns: [{field:'ProcessList_Id',title:'工序采集数据主键ID',type:'int',width:110,hidden:true,readonly:true,require:true,align:'left'},
                       {field:'Process_Id',title:'工序',type:'int',bind:{ key:'processKey',data:[]},width:110,require:true,align:'left'},
  ...
  // new codes
  mounted() {
    // Load dictionary data for bind keys
    const keys = ['dataPointType', 'processKey'];
    this.http.post('/api/Sys_Dictionary/GetVueDictionary', keys).then((dic) => {
      dic.forEach(d => {
        this.columns.forEach(col => {
          if (col.bind && col.bind.key === d.dicNo) {
            // Use $set to ensure reactivity for nested object
            this.$set(col.bind, 'data', d.data);
          }
        });
      });
    }).catch(err => {
