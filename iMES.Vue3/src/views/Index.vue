  ...
  <div id="mes-container" :class="['mes-theme-' + theme]">
    ...
    <div class="mes-container" :style="{ left: menuWidth - 1 + 'px' }">
      <div class="mes-header">
        <div class="project-name">{{ $t("【商业版】有意联系微信：lovezaomeng") }}</div>
        ...
        <div class="header-info">
          <div class="h-link">
            <el-select v-model="language" @change="changeLanguage($event)" placeholder="请选择语言">
              <el-option
          ...
          >
            <li v-show="visibleItem.all">
              <el-button link @click="closeTabs()">
                ...
                {{
                  navigation.length == 2 ? $t(closeMenu) : "关闭所有"
                }}</el-button
              >
            </li>
...
<script>
  ...
  import zhCN from "../language/locales/zh-CN";
import enUS from "../language/locales/en-US";
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const state = reactive({
      name: i18nLanguage.global.t("headTxt.user.name"),
      computedName: computed(() => i18nLanguage.global.t("headTxt.user.name")),
      userName: computed(() => i18nLanguage.global.t("姓名")),
    });
    ...
    const changeLanguage = (lang) => {
      locale.value = lang == "zh-CN" ? "zh-CN" : "en-US"; // 切换中英文
      localStorage.setItem("language",lang);
      var zh = {};
      var en = {};
      // console.log("1111",zhCN);
      http.post('/api/Base_Language/getPageData', {}, true).then((result) => {
          console.log("2222",result)
                    ...
                    var languagepack = lang == "zh-CN" ? zh : en;
                    setLocaleMessage(lang, languagepack); // 设置语言包
                    location.reload();
                });
    ...
    const links = ref([
      ...
      {
        text: "Java版本演示",
        path: "https://java.625sc.com:8443/imes",
        id: -2,
      },
      {
        text: "Java版本介绍",
        path: "https://www.625sc.com/imes/333.html",
        id: -2,
      },
   ...
   */
  mounted() {
    let _lan = localStorage.getItem('language');
      if(_lan)
      {
        this.language = _lan; 
        locale.value = _lan; // 切换中英文】
        var zh = {};
        var en = {};
        http.post('/api/Base_Language/getPageData', {}, true).then((result) => {
                    result.rows.forEach(lan => {
                    ...
                    var languagepack = _lan == "zh-CN" ? zh : en;
                    setLocaleMessage(_lan, languagepack); // 设置语言包
                });
      }
      else{
        this.language =  "zh-CN"; 
        locale.value =  "zh-CN"; // 切换中英文
